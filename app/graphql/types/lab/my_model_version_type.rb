        ...
        dataset.items.map do |item|
          {
            ...
            trigger_status: fetch_trigger_status(model.model_id, model.version_id, item["label"], item["level"]),
            trigger_updated_at: fetch_trigger_updated_at(model.model_id, model.version_id, item["label"], item["level"]),
            logo_url: extract_logo_url(item["label"])
          }
        end
      end

      def fetch_trigger_status(model_id, version_id, project_url,level)
        CustomAnalyzeProjectServer.new({ user: nil, model: LabModel.find_by(id: model_id), version: LabModelVersion.find_by(id: version_id), project: project_url, level: level }).check_task_status
      end

      def fetch_trigger_updated_at(model_id, version_id, project_url,level)
        CustomAnalyzeProjectServer.new({ user: nil, model: LabModel.find_by(id: model_id), version: LabModelVersion.find_by(id: version_id), project: project_url, level: level }).check_task_updated_time
      end
