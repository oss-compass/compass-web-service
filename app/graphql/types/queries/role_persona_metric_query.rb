...
module Types
  module Queries
    class RolePersonaMetricQuery < BaseQuery
      type [Types::RolePersonaMetricType], null: false
      description 'Get role persona metrics'
      ...
      def resolve(label: nil, level: 'repo', repo_type: nil, begin_date: nil, end_date: nil)
        ...
        if !interval
          resp = RolePersonaMetric.query_repo_by_date(label, begin_date, end_date, page: 1, type: repo_type)

          build_metrics_data(resp, Types::RolePersonaMetricType) do |skeleton, raw|
            skeleton.merge!(raw)
            RolePersonaMetric.fields_aliases.map { |alias_key, key| skeleton[alias_key] = raw[key] }
          ...
          aggs = generate_interval_aggs(
            Types::RolePersonaMetricType,
            :grimoire_creation_date,
            interval,
            'Float',
            RolePersonaMetric.fields_aliases
          )
          resp = RolePersonaMetric.aggs_repo_by_date(label, begin_date, end_date, aggs, type: repo_type)
