...
module Types
  module Queries
    class DomainPersonaMetricQuery < BaseQuery
      type [Types::DomainPersonaMetricType], null: false
      description 'Get domain persona metrics'
      ...
      def resolve(label: nil, level: 'repo', repo_type: nil, begin_date: nil, end_date: nil)
        ...
        if !interval
          resp = DomainPersonaMetric.query_repo_by_date(label, begin_date, end_date, page: 1, type: repo_type)

          build_metrics_data(resp, Types::DomainPersonaMetricType) do |skeleton, raw|
            skeleton.merge!(raw)
            DomainPersonaMetric.fields_aliases.map { |alias_key, key| skeleton[alias_key] = raw[key] }
          ...
          aggs = generate_interval_aggs(
            Types::DomainPersonaMetricType,
            :grimoire_creation_date,
            interval,
            'Float',
            DomainPersonaMetric.fields_aliases
          )
          resp = DomainPersonaMetric.aggs_repo_by_date(label, begin_date, end_date, aggs, type: repo_type)
