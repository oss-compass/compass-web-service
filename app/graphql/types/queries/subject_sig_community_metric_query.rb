...
module Types
  module Queries
    class SubjectSigCommunityMetricQuery < BaseQuery
      ...
      def resolve(label: nil, level: 'repo', begin_date: nil, end_date: nil)
        ...
        aggs = {
          date_ranges: {
            ...
            aggs: {
              avg_score: { avg: { field: CommunityMetric.main_score } }
            }
          }
        }
        ...
        subjects.map do |subject|
          resp = CommunityMetric.aggs_repo_by_date(subject.label, begin_date, end_date, aggs, type: repo_type)
