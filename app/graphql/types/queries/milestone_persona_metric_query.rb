...
module Types
  module Queries
    class MilestonePersonaMetricQuery < BaseQuery
      type [Types::MilestonePersonaMetricType], null: false
      description 'Get milestone persona metrics'
      ...
      def resolve(label: nil, level: 'repo', repo_type: nil, begin_date: nil, end_date: nil)
        ...
        if !interval
          resp = MilestonePersonaMetric.query_repo_by_date(label, begin_date, end_date, page: 1, type: repo_type)

          build_metrics_data(resp, Types::MilestonePersonaMetricType) do |skeleton, raw|
            skeleton.merge!(raw)
            MilestonePersonaMetric.fields_aliases.map { |alias_key, key| skeleton[alias_key] = raw[key] }
          ...
          aggs = generate_interval_aggs(
            Types::MilestonePersonaMetricType,
            :grimoire_creation_date,
            interval,
            'Float',
            MilestonePersonaMetric.fields_aliases
          )
          resp = MilestonePersonaMetric.aggs_repo_by_date(label, begin_date, end_date, aggs, type: repo_type)
