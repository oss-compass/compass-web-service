...
module Mutations
  module Tpc
    class UpdateTpcSoftwareLectotypeReport < BaseMutation
      include CompassUtils

      field :status, String, null: false

      argument :report_id, Integer, required: true
      argument :software_report, Input::TpcSoftwareLectotypeReportUpdateInput, required: true
      ...
      def resolve(report_id: nil, software_report: nil)
        ...
        ActiveRecord::Base.transaction do
          lectotype_report.update!(software_report.as_json(except: [:architecture_diagrams]))
          ...
          if keep_images.present?
            diagrams_to_delete = lectotype_report.architecture_diagrams.where.not(id: keep_images)
            diagrams_to_delete.each do |diagram|
              diagram.purge
            end
          else
            lectotype_report.architecture_diagrams.purge
          end
          ...
          if new_images.present?
            ...
            lectotype_report.architecture_diagrams.attach(diagrams_to_attach)
          end
        end
