module Mutations
  class AppendKeywordToProject < BaseMutation
    include Common
    graphql_name 'AppendKeywordToProject'
    ...
    def resolve(label:, keyword_id:, token:)
      ...
      ProjectKeywordRef.create!(project_name: label, keyword_id: keyword.id)
      ...
      OpenStruct.new(
        result.merge(
          {
            status: false,
            message: I18n.t('project.append_keyword_failed', reason: ex.message)
          }
        )
      )
    end
