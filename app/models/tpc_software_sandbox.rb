# == Schema Information
#
# Table name: tpc_software_sandboxes
#
#  id                              :bigint           not null, primary key
#  tpc_software_sandbox_report_ids :string(255)      not null
#  repo_url                        :string(255)
#  committers                      :string(255)      not null
#  adaptation_committers           :string(255)
...
#
class TpcSoftwareSandbox < ApplicationRecord

  belongs_to :subject
  belongs_to :user

  belongs_to :tpc_software_sandbox_report, foreign_key: 'target_software_report_id', class_name: 'TpcSoftwareSandboxReport'
  belongs_to :tpc_software_report, foreign_key: 'target_software_report_id', class_name: 'TpcSoftwareSandboxReport'
  ...
  def self.get_review_permission(sandbox, member_type)
    clarify_metric_list = TpcSoftwareSandbox::Clarify_Metric_List

    target_metric = TpcSoftwareSandboxReportMetric.where(tpc_software_sandbox_report_id: JSON.parse(sandbox.tpc_software_sandbox_report_ids))
                                                  .where(tpc_software_sandbox_report_type: TpcSoftwareSandboxReportMetric::Report_Type_Sandbox)
                                                  .where(version: TpcSoftwareSandboxReportMetric::Version_Default)
    ...
    comment_state_list = TpcSoftwareCommentState.where(tpc_software_id: target_metric.id)
                                                .where(tpc_software_type: TpcSoftwareCommentState::Type_Sandbox_Report_Metric)
  ...
  def self.get_risk_metric_list(report_id)
    target_metric = TpcSoftwareSandboxReportMetric.where(tpc_software_sandbox_report_id: report_id)
                                                  .where(tpc_software_sandbox_report_type: TpcSoftwareSandboxReportMetric::Report_Type_Sandbox)
                                                  .where(version: TpcSoftwareSandboxReportMetric::Version_Default)
                                                  .take
    ...
    risk_metric_list = []
    TpcSoftwareSandbox::Clarify_Metric_List.each do |clarify_metric|
      score = target_metric_hash[clarify_metric]
      if score.present? && (0 <= score) && (score < 10)
  ...
  def self.get_clarified_metric_list(report_id)
    ...
    comment_list = TpcSoftwareComment.where(tpc_software_id: target_metric.id)
                                     .where(tpc_software_type: TpcSoftwareComment::Type_Sandbox_Report_Metric)
  ...
  def self.get_comment_state_list(sandbox_id)

    TpcSoftwareCommentState.where(tpc_software_id: sandbox_id)
                           .where(metric_name: TpcSoftwareCommentState::Metric_Name_Sandbox)
  ...
  def self.get_upstream_collaboration_strategy_score(report_id)
    # code here
    report_metric = TpcSoftwareSandboxReportMetric.where(tpc_software_sandbox_report_id: report_id)
  ...
  def self.get_current_state(tpc_software)
    ...
    required_member_types = TpcSoftwareCommentState::Sandbox_Member_Types
  ...
  def self.update_state(id)
    sandbox = TpcSoftwareSandbox.find_by(id: id)
    state = get_current_state(sandbox)
    ...
    if state == State_Completed
      ...
      if create_msg == true
        sandbox.update!(state: State_Repo_Created)
      else
        sandbox.update!(state: State_Repo_Created_Error, remark: create_msg)
      end
    end
  ...
  def self.save_issue_url(id, issue_html_url)
    selection = TpcSoftwareSandbox.find_by(id: id)
