# == Schema Information
#
# Table name: tpc_software_sandbox_reports
#
...
#
class TpcSoftwareSandboxReport < ApplicationRecord
  ...
  belongs_to :subject
  belongs_to :user
  has_many :tpc_software_sandbox_report_metrics, as: :tpc_software_sandbox_report, dependent: :destroy
  has_many :tpc_software_sandboxes, foreign_key: 'target_software_sandbox_report_id'

  has_many_base64_attached :attachments

  Report_Type_Sandbox = 0
  ...
  def self.generate_short_code
    loop do
      ...
      break short_code unless TpcSoftwareSandboxReport.exists?(short_code: short_code)
    end
  end
end
