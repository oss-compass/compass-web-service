...
module Openapi
  module Entities
    class EcologyInterfaceDocItem < Grape::Entity
      ...
      expose :ecology_interface_doc,
             documentation: {
               type: 'Integer',
               desc: 'Interface doc score / 接口文档分数',
               example: 0
             }

      expose :ecology_interface_doc_detail,
             documentation: {
               type: 'String',
               desc: 'Interface doc detail (nullable) / 接口文档详情（可为空）',
               example: nil,
               nullable: true
             }
      ...
      expose :has_api_docs,
             documentation: {
               type: 'Boolean',
               desc: 'Whether API docs exist / 是否存在API接口文档',
               example: false
             }

      expose :ecology_interface_doc_raw,
             documentation: {
               type: 'String',
               desc: 'Raw interface doc check result (JSON string) / 接口文档检查原始结果（JSON字符串）',
               example: '[]'
             }
    end

    class EcologyInterfaceDocResponse < Grape::Entity
      ...
      expose :items, using: Entities::EcologyInterfaceDocItem,
             documentation: { type: 'Entities::EcologyInterfaceDocItem', desc: 'response', param_type: 'body', is_array: true }
    end
  end
end
