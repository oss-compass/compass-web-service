      ...
      class CommunityPortrait < Grape::API
        ...
        resource :community_portrait do
          ...
          desc 'Code Maintenance Status / 代码是否维护',
               ...
               tags: ['V2 API', 'Metrics Data / 指标数据', 'Community Persona / 社区画像'],
               success: {
          ...
          desc 'Contributor Code Lines in Time Range / 指定时间段内贡献者代码量',
               detail: 'Lines of code (added plus removed) contributed by each contributor in the specified time range / 指定时间段内每个贡献者贡献的代码行数 (增加行数加上删除行数)',
               tags: ['V2 API', 'Metrics Data / 指标数据', 'Community Persona / 社区画像'],
               success: {
                 code: 201, model: Openapi::Entities::ContributorCodeLinesInRangeResponse
               }
          params {
            requires :access_token, type: String, desc: 'Access token / 访问令牌', documentation: { param_type: 'body' }
            requires :label, type: String, desc: 'Repository or community URL / 仓库或社区地址', documentation: { param_type: 'body', example: 'https://github.com/oss-compass/compass-web-service' }
          ...
          post :contributor_code_lines_in_range do
            label = params[:label]
            level = params[:level] || 'repo'
            begin_date = params[:begin_date]
            end_date = params[:end_date]
            ...
            resp = indexer.fetch_contributor_code_lines_by_repo_urls(repo_urls, begin_date, end_date)
            buckets = resp&.[]('aggregations')&.[]('group_by_name')&.[]('buckets') || []

            code_lines_list = buckets.map do |bucket|
              {
                'contributor' => bucket.dig('author_name', 'hits', 'hits', 0, '_source', 'author_name') || bucket['key'],
                'author_email' => bucket['key'],
                'code_additions' => bucket.dig('lines_added', 'value').to_i,
                'code_deletions' => bucket.dig('lines_removed', 'value').to_i,
                'code_lines' => bucket.dig('lines_changed', 'value').to_i
              }
            end

            count = code_lines_list.size
            items = code_lines_list[(page - 1) * size, size] || []
