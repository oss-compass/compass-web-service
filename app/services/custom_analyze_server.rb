...
class CustomAnalyzeServer
  ...
  attr_reader :user, :model, :version
  ...
  def initialize(opts = {user:, model:, version: })
    @user = opts[:user]
    @model = opts[:model]
  ...
  def project_urls
    version.dataset.items.map { |item| item['label'] }
  end
  ...
  def analysis_task_key
    "#{self.class.name}:#{model.id}:#{version.id}:task_status"
  end

  def analysis_response_key
    "#{self.class.name}:#{model.id}:#{version.id}:task_response"
  end
  ...
  def update_task_status(task_id)
    # Rails.cache.fetch(analysis_response_key, expires_in: CacheTTL) do
