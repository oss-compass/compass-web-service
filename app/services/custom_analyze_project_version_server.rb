...
class CustomAnalyzeProjectVersionServer
  PROJECT = 'insight'
  WORKFLOW = 'CUSTOM_V1'
  LongCacheTTL = 3.days
  CacheTTL = 2.minutes
  ...
  attr_reader :user, :model, :version, :project, :version_number, :level
  ...
  def initialize(opts = { user:, model:, version:, project:, version_number:, level: })
    @user = opts[:user]
    @model = opts[:model]
  ...
  def custom_fields
    { model_id: model.id, version_id: version.id, version_number: version_number }
  end
  ...
  def analysis_response_key
    "#{self.class.name}:#{model.id}:#{version.id}:#{project}:#{version_number}:task_response"
  end
